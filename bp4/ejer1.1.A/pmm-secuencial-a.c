...
int main(int argc, char ** argv)
{
	...
	double total;
	double mult0, mult1, mult2, mult3;
	mult0 = mult1 = mult2 = mult3 = 0;
	...
	for(i=0; i<N; i++)
		for(j=0; j<N; j++){
			mult0 = mult1 = mult2 = mult3 = 0;
				for(k=0; k<N; k+=4){
					mult0 += m1[i][k]*m2[k][j];
					mult1 += m1[i][k+1]*m2[k+1][j];
					mult2 += m1[i][k+2]*m2[k+2][j];
					mult3 += m1[i][k+3]*m2[k+3][j];
				}
				m3[i][j] = mult0 + mult1 + mult2 + mult3;
			}
